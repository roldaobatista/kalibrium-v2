...
- **AC-004:** Dado que um auditor/gate sub-agent recebe um prompt que passa por `validate-audit-prompt.sh` mas que ainda é percebido como contaminado, quando processa o prompt, então o agent file instrui recusa emitindo JSON conforme o schema canônico: `verdict: "rejected"` + campo adicional `rejection_reason: "contaminated_prompt"` + `contamination_evidence: "<token ou passagem>"`. Teste verifica: (a) JSON passa pelo `validate-gate-output.sh` (schema-valid); (b) `rejection_reason == "contaminated_prompt"`; (c) o JSON de rejeição NÃO contém os campos `evidence.ac_coverage_map`, `evidence.checks` nem qualquer finding técnico além da `contamination_evidence` (verificação mecânica: `jq '(.evidence // {} | has("ac_coverage_map") or has("checks"))'` retorna `false`) — provando que o agente abortou antes de investigar artefatos.
