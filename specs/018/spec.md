...
- **AC-002:** Dado que um commit toca um arquivo compartilhado (lista fechada em `scripts/detect-shared-file-change.sh`: `src/main.tsx`, `vite.config.ts`, `package.json`, `capacitor.config.ts`, `playwright.config.ts`, `.claude/settings*.json`), quando `pre-push` hook roda, então executa `scripts/detect-shared-file-change.sh` (convenção Unix: **exit 0 = operação bem-sucedida, imprime em stdout `shared_changed=true` se algum arquivo compartilhado foi alterado em `git diff --name-only @{push}..HEAD`, ou `shared_changed=false` caso contrário**; exit ≠ 0 só ocorre em erro interno do script) e, se stdout contém `shared_changed=true`, dispara `scripts/smoke-tests.sh` que executa a tag `@smoke` (10-15 testes críticos cobrindo: auth, scaffold render, PWA offline) e bloqueia push se qualquer teste falhar (exit ≠ 0 de `smoke-tests.sh`).
- **AC-002-A:** Dado um commit que NÃO toca arquivo compartilhado (`detect-shared-file-change.sh` imprime `shared_changed=false` com exit 0), quando `pre-push` roda, então o smoke não é executado — push prossegue direto. Teste: commit de 1 arquivo de `docs/` verifica que (a) script retorna `shared_changed=false`, (b) pre-push passa sem disparar smoke.
...
- **AC-004:** Dado que um auditor/gate sub-agent recebe um prompt que passa por `validate-audit-prompt.sh` mas que ainda é percebido como contaminado, quando processa o prompt, então o agent file instrui recusa emitindo JSON conforme o schema canônico: `verdict: "rejected"` + campo adicional `rejection_reason: "contaminated_prompt"` + `contamination_evidence: "<token ou passagem>"`. Teste verifica: (a) JSON passa pelo `validate-gate-output.sh` (schema-valid); (b) `rejection_reason == "contaminated_prompt"`; (c) o JSON de rejeição NÃO contém os campos `evidence.ac_coverage_map`, `evidence.checks` nem qualquer finding técnico além da `contamination_evidence` (verificação mecânica: `jq 'has("evidence.ac_coverage_map") or has("evidence.checks")'` retorna `false`) — provando que o agente abortou antes de investigar artefatos.
...
- **AC-005:** Dado que um gate sub-agent emite seu JSON final, quando `scripts/validate-gate-output.sh specs/NNN/<arquivo>.json` é executado, então exige literal `"$schema": "gate-output-v1"`, `"slice": "NNN"` no formato `^[0-9]{3}$`, e `"gate"` pertencente ao enum canônico completo definido em `docs/protocol/schemas/gate-output.schema.json` (18 valores incluindo `verify`, `review`, `security-gate`, `audit-tests`, `functional-gate`, `master-audit`, `audit-spec`, `plan-review`, e os gates opcionais `data-gate`, `observability-gate`, `integration-gate`, `ux-gate`, `ci-gate`, `audit-tests-draft`, `audit-story`, `audit-planning`, `spec-security`, `guide-audit`). Validator lê o enum direto do schema (não hardcoded). JSON fora desse contrato é rejeitado com mensagem clara apontando campo violador.
