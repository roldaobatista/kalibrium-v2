...
- **AC-002 — Build web produz artefatos sem erros**
  Dado o repositório com dependências instaladas, quando o desenvolvedor executa `npm run build`, então o diretório `dist/` é gerado com `index.html` e assets (`dist/assets/`), e o processo termina com exit 0.
...
- **AC-001a — `npm run dev` falha com mensagem clara se porta 5173 estiver ocupada**
  Dado que a porta 5173 está ocupada por outro processo, quando o desenvolvedor executa `npm run dev`, então Vite emite mensagem informando a porta ocupada e não sobe na mesma porta silenciosamente (comportamento padrão Vite: tenta porta alternativa e anuncia; aceitável desde que a nova porta seja impressa no stdout).

- **AC-002a — Build falha com erro se `tsconfig.json` tiver erro de tipo**
...
- **AC-005a — Scaffold não cria diretórios extras fora do escopo declarado**
...
- **AC-007a — ESLint detecta violação em arquivo seed e falha**
...
- **AC-008a — `routes/web.php` limpo pós-descarte**
...
- **AC-SEC-001 — `capacitor.config.ts` não expõe servidor remoto de dev em produção**
