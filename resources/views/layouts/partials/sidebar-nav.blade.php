{{--
    Navegação da sidebar — usado tanto no drawer mobile quanto na sidebar desktop.
    Variável: $mobile (bool) — controla @click="sidebarOpen = false" nos links mobile.
--}}
@php
    $navItems = [
        [
        ...
        [
            ...
            'active' => request()->routeIs('technicians.*'),
            'icon'   => '<path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />',
        ],
        [
            'label'  => 'Celulares dos técnicos',
            'route'  => route('mobile-devices.index'),
            'active' => request()->routeIs('mobile-devices.*'),
            'icon'   => '<path stroke-linecap="round" stroke-linejoin="round" d="M10.5 1.5H8.25A2.25 2.25 0 0 0 6 3.75v16.5a2.25 2.25 0 0 0 2.25 2.25h7.5A2.25 2.25 0 0 0 18 20.25V3.75a2.25 2.25 0 0 0-2.25-2.25H13.5m-3 0V3h3V1.5m-3 0h3m-3 8.25h3" />',
        ],
        [
...
<nav class="flex-1 py-4" role="navigation" aria-label="Navegação principal">
            ...
            <li>
                <a
                    ...
                    class="flex items-center gap-3 px-3 py-3 rounded-md text-sm font-medium transition-colors
                        {{ $item['active']
                            ? 'bg-primary-800 text-white border-l-4 border-accent-500 pl-2'
                            : 'text-primary-200 hover:bg-primary-800 hover:text-white' }}"
                    aria-current="{{ $item['active'] ? 'page' : 'false' }}"
                    @if($mobile ?? false) @click="sidebarOpen = false" @endif
                >
                    <svg class="w-5 h-5 flex-shrink-0 {{ $item['active'] ? 'text-white' : 'text-primary-300' }}"
