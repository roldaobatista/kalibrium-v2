...
> Documento normativo mestre. versao 1.2.4 — 2026-04-17.
> Status: aprovado pelo PM.
> Changelog 1.2.4 (MINOR — ADR-0017 Mudanca 1): introduz o gate `audit-tests-draft` entre `draft-tests` e a implementacao da Fase D. `qa-expert` em modo `audit-tests-draft` audita testes red com rastreabilidade AC-ID obrigatoria (cada teste cobre um AC via nome, `@covers AC-NNN` ou `->group('slice-NNN','ac-NNN')`). Enum do schema `gate-output.schema.json` expandido. Primeiro slice completo sob 1.2.4 = slice-015 (Spike INF-007).
> Changelog 1.2.3 (MINOR — gates L4 pre-reviews): adicionados gates de planejamento auditaveis (`plan-review`, `spec-audit`) com criterios em 04 §§10-15. Politica dual-LLM: dois contextos isolados podem usar o mesmo modelo (2xOpus) ou vendors distintos, desde que R3/R11 sejam respeitados. Consolida ganhos pre-ADR-0017 em documento formal.
