...
Aplicam-se aos artefatos emitidos pelos **gates do pipeline** (16 gates do protocolo v1.2.2) e pelas skills de governança que seguem contrato mecânico. Todo gate que o protocolo enumera como `approved` | `rejected` | `needs_changes` valida sua saída JSON contra um destes arquivos.

| Arquivo | Escopo de aplicação | Consumidores |
|---|---|---|
| `gate-output.schema.json` | Todos os **16 gates** do pipeline v1.2.2+ADR-0017: verify, code-review, security-review, audit-tests-draft (pre-implementacao — ADR-0017 Mudanca 1), audit-tests (pos-implementacao), functional-review, data-review, observability-review, integration-review, ux-review, master-audit, audit-spec, audit-story, audit-planning, plan-review, spec-security (L4), guide-audit, ci-gate | `verify-slice`, `review-pr`, `security-review`, `audit-tests-draft`, `test-audit`, `functional-review`, `data-review`, `observability-review`, `integration-review`, `ux-review`, `master-audit`, `audit-spec`, `audit-story`, `audit-planning`, `review-plan`, `ci-gate` |
...
│       ├── gate-output.schema.json             # 16 gates do pipeline v1.2.2
...
- `docs/protocol/04-criterios-gate.md` — critérios objetivos dos 16 gates (§§1-16) + regras transversais (§17)
