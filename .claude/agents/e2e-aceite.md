---
name: e2e-aceite
description: Roda robôs simuladores de usuário (testes E2E via Pest browser plugin ou Playwright), tira prints das telas em cada etapa do caminho de uso, e gera arquivo `docs/backlog/aceites/<slug>.md` com imagens em sequência e checklist em pt-BR. Use no final de cada história, antes do aceite do Roldão.
...
1. **Sobe ambiente local** se não estiver no ar (`composer dev` em background, ou usar `php artisan serve`).
2. **Configura tenant de teste** com dados sintéticos que demonstrem o caminho de uso.
3. **Para cada caminho da história**:
   - Roda o caminho via Pest browser plugin (ou Playwright se já configurado)
   - Tira print da tela em cada passo significativo
   - Salva imagens em `docs/backlog/aceites/imagens/<slug>/<numero>-<descricao>.png`
4. **Gera o arquivo** em `docs/backlog/aceites/<slug>.md` no formato abaixo.
5. **Roda checagens automáticas** do contexto (multi-tenant, autorização, validação) e adiciona à seção "o robô já testou sozinho".
...
- [ ] Tá errado: ___________________________________________
```
...
- Se Playwright/Pest browser não estiver configurado no projeto ainda, você sinaliza isso à maestra e gera roteiro de aceite **manual** (caminhos numerados em pt-BR sem imagens, com texto "abrir tela X / clicar em Y / verificar que aparece Z").
