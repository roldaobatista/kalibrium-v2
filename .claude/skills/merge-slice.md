---
description: Merge final do slice após os cinco gates obrigatórios terem aprovado. Valida verifier, reviewer, security-reviewer, test-auditor e functional-reviewer, prepara PR body e — se permissões de push estiverem liberadas — executa git push + gh pr create. Se o push ainda estiver selado, imprime o roteiro exato para o PM. Uso: /merge-slice NNN.
---
...
## Quando invocar
Após `/verify-slice NNN`, `/review-pr NNN`, `/security-review NNN`, `/test-audit NNN` e `/functional-review NNN` terem emitido `verdict: approved` e findings vazios. Nunca antes — R11 exige dupla-aprovação independente e o pipeline atual exige os três gates finais antes de merge.
...
- `specs/NNN/verification.json` existe, tem `verdict: approved` e `violations: []`
- `specs/NNN/review.json` existe, tem `verdict: approved` e `findings: []`
- `specs/NNN/security-review.json` existe, tem `verdict: approved`, `findings: []` e `severity_summary` zerado
- `specs/NNN/test-audit.json` existe, tem `verdict: approved`, `findings: []`, `anti_patterns: []` e todos os ACs cobertos
- `specs/NNN/functional-review.json` existe, tem `verdict: approved`, findings vazios e todos os ACs funcionais atendidos
...
1. **Gates obrigatórios:** lê `verification.json`, `review.json`, `security-review.json`, `test-audit.json` e `functional-review.json`. Qualquer gate ausente, rejeitado ou com findings aborta com exit 1.
2. **Integridade do harness:** drift em settings ou hooks = abort.
3. **Gera PR body** em `specs/NNN/pr-body.md` com resumo dos ACs verificados, referência aos cinco gates, e bloco R12 em linguagem de PM.
