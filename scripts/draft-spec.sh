...
# ---------------------------------------------------------------------------
for section in "Contexto" "Jornada alvo" "Acceptance Criteria" "Fora de escopo"; do
  if awk -v s="$section" '
  ...
  ' "$SPEC"; then
    ...
    fail "seção '$section' vazia ou sem conteúdo substantivo"
  fi
done
...
# ---------------------------------------------------------------------------
AC_LINES="$(grep -nE '^\s*-\s*\*?\*?AC-[0-9]+' "$SPEC" || true)"
AC_COUNT=0
if [ -n "$AC_LINES" ]; then
...
# 4. ACs sem descrição
# ---------------------------------------------------------------------------
EMPTY_AC="$(grep -nE '^\s*-\s*\*?\*?AC-[0-9]+\*?\*?:?\s*$' "$SPEC" || true)"
if [ -n "$EMPTY_AC" ]; then
...
# ---------------------------------------------------------------------------
if grep -nE '^\s*-\s*\*?\*?AC-[0-9]+.*(TODO|TBD|FIXME|\.\.\.)' "$SPEC" >/dev/null; then
